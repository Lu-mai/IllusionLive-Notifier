...
1. [최신 릴리스](https://github.com/Lu-mai/IllusionLive-Notifier/releases/latest)에서 `IllusionLiveNotifier.apk`를 Android 기기로 내려받아 실행
...
이 저장소 오른쪽 위 **Watch → Custom → Releases** 를 켜 두면 새 버전이 올라올 때 GitHub 알림을 받습니다 (GitHub 계정 필요). 앱에는 업데이트 확인 기능이 없어서, 알림을 받으면 릴리스 페이지에서 새 APK를 내려받아 그대로 덮어 설치하면 됩니다. 설정은 유지됩니다.
...
댓글 알림은 올라온 지 사흘이 지나지 않은 글에서만 동작합니다. 나중에 단 내 댓글에 달린 답은 그중 최근 글 약 20개에서 찾습니다.

## Windows 버전

새 글을 알려 주는 Windows 트레이 앱도 함께 들어 있습니다(댓글 알림은 없음). 요구 사항: Windows 10/11 x64, .NET 10 SDK
